...
export default function Warehouse2DView() {
  ...
  // ───────────────────────────────────────────────
  // Ao clicar no nível, abre modal
  // ───────────────────────────────────────────────
  const handleNivelClick = (fileira: Fileira, grade: Grade, nivel: Nivel) => {
    setNivelSelecionado({ fileira, grade, nivel });
    setShowModal(true);
  };
            ...
            {nivelSelecionado ? (
              <View style={styles.modalDetailsWrapper}>
                ...
                <Text style={styles.modalText}>
                  ...
                  {nivelSelecionado.fileira.identificador}
                </Text>

                <Text style={styles.modalText}>
                  <Text style={styles.label}>Grade: </Text>
                  {nivelSelecionado.grade.identificador}
                </Text>

                <Text style={styles.modalText}>
                  <Text style={styles.label}>Nível: </Text>
                  {nivelSelecionado.nivel.identificador}
                </Text>

                <Text style={styles.modalText}>
                  <Text style={styles.label}>Código Wester: </Text>
                  {nivelSelecionado.nivel.id}
                </Text>

                <Text style={styles.modalText}>
                  <Text style={styles.label}>Quantidade: </Text>
                  {nivelSelecionado.nivel.quantidade ?? 0}
                </Text>
              </View>
            ) : (
              <Text style={styles.modalText}>Nenhum nível selecionado.</Text>
            )}
...
export default function Warehouse2DView() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error('Erro ao buscar estrutura do estoque:', err);
      }
    }
  ...
  const toggleGradeExpand = (grade: Grade) => {
    ...
    setWidthAnims((prev) => ({ ...prev, [grade.id]: anim }));
  };
